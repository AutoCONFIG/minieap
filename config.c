...
#define COPY_N_ARG_TO(buf, maxlen) \
    ...
    if (ISOPT("username")) {
        ...
        if (strcmp(argument, "0") == 0)
            g_prog_config.kill_type = KILL_NONE; /* 结束其他实例并退出 */
        else if(strcmp(argument, "1") == 0)
            g_prog_config.kill_type = KILL_ONLY; /* 结束其他实例，本实例继续运行 */
...
#define ASSERT_NOTIFY(x, msg) \
    ...
    ASSERT_NOTIFY(g_proxy_config.proxy_on && !g_proxy_config.lan_ifname,"代理认证开启时，LAN 侧网卡名不能为空");
