...
void rjv3_print_cmdline_help(struct _packet_plugin* this) {
    PR_RAW(
        ...
        "\t--dhcp_type, -d <0-3>\t\tDHCP 方式： [默认" STR(DEFAULT_DHCP_TYPE) "]\n"
            ...
            "\t\t\t\t\t2 = 认证后 DHCP\n"
            "\t\t\t\t\t3 = 认证前 DHCP\n"
        //"\t--loacl-packet, -lp\t\t使用抓包文件：[默认目录"STR(DEFAULT_LOCAL_PACKET)"，没有此目录请自行创建]"
        "\t--dhcp_script, -c <...>\t\t二次认证之间及认证完成后运行此命令 [默认无]\n"
...
RESULT rjv3_process_cmdline_opts(struct _packet_plugin* this, int argc, char* argv[]) {
    ...
    static const struct option longOpts[] = {
	    ...
	    { "dhcp_script", required_argument, NULL, 'c' },
...
void rjv3_save_config(struct _packet_plugin* this) {
    ...
    conf_parser_add_value("dhcp_script", PRIV->dhcp_script);
...
static void packet_plugin_rjv3_print_banner() {
    PR_INFO("\nRJv3 for MiniEAP " VERSION "\n"
            ...
            "Hamster Tian & Taiga（Old name Kana), 2021\n"
            "支持RG-SU For Linux V1.30特征字段 @Taiga\n\n");
}
