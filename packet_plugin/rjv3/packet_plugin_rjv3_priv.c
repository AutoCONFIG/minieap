...
static void rjv3_set_v3_hash(uint8_t* hash_buf/*, ETH_EAP_FRAME* request*/) {
    //uint8_t* _v3_buf;
    static char _hash[]={
        
    };
    /*
    ...
    memmove(hash_buf, _hash, sizeof(_hash));//It's static hash in RG-SU For Linux V1.31, so...
}
...
static int rjv3_append_common_fields(PACKET_PLUGIN* this, LIST_ELEMENT** list, int append_pwd_hash) {
    ...
    uint8_t _end_str[RJV3_SIZE_END_STR] = {0x02};//Creat RG-SU For Linux V1.30 End string
...
#define CHK_ADD(x) \
    ...
    CHK_ADD(append_rjv3_prop(list, RJV3_TYPE_END_STR,  _end_str,               sizeof(_end_str)));//Send RG-SU For linux V1.30 End string
...
static void rjv3_append_priv_header(struct _packet_plugin* this, ETH_EAP_FRAME* frame) {
    ...
    *(uint16_t*)&_dhcp_prop.crc16_hash = htons(crc16((uint8_t*)&_dhcp_prop, 21));
    ...
    append_to_frame(frame, (uint8_t*)&_dhcp_prop, 23);
